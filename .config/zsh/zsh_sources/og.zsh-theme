...
local return_code="${bg_return_code}%(?.%F{46}%K{black}✔ %f%F{15}.${fg_return_code} %? ↵ %F{15}%K{1}%f)%k"
...
local current_dir="%B${bg_wd}${fg_wd}  %~%f %b%k${wd_separator}"

local ruby=" \$(ruby_prompt_info)"
local git_branch="$(git_prompt_info)${git_end}"

PROMPT="
${apple_symbol}${user_host}${current_dir}%E
$PR_PROMPT "
RPROMPT="${rprompt_end}${return_code}${command_count}${time}${rprompt_start}"

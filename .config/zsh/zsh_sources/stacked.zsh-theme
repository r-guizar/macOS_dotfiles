...
if [[ -n $(git_prompt_info) ]]; then
  local wd_separator="%F{39}${bg_git}%k"
...
local reverse_return_code="${bg_return_code}%(?.%F{46}%k ✔%f%F{15}.${fg_return_code} %? ↵ %F{1}%k%f)%k"
...
local current_dir_reverse_return_code="%B%F{39}%f${bg_wd}${fg_wd} %~%f %b%k${wd_separator}"

local ruby=" \$(ruby_prompt_info)"
local git_branch="$(git_prompt_info)${git_end}"

PROMPT="
${apple_symbol}${user_host}${reverse_return_code}
${current_dir_reverse_return_code}%E
$PR_PROMPT "
